...
Write-Host $Root
$Solution = Get-ChildItem $Root "FluentMsBuild.sln" -File
...
$FluentMsBuild = Get-ChildItem $Sources "FluentMsBuild" -Directory
Write-Host $FluentMsBuild.FullName
$FluentMsBuildProj = Get-ChildItem $FluentMsBuild.FullName "FluentMsBuild.csproj" -File
Write-Host $FluentMsBuildProj.FullName

#goto :eof
& $NuGet restore $Solution.FullName

& msbuild /t:Pack /p:Configuration=Release $FluentMsBuildProj.FullName /p:NuGetExePath=$NuGet